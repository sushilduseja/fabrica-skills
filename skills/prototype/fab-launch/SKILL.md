...
10. If pre-launch checklist fails, set `last_error = { "type": "gate_blocked", "message": "Pre-launch checklist failed: <details>" }`, set `next_action = "/fab-launch"`, and halt.
11. Set `current_step = "fab-launch"`, bump `updated_at`, append verification, and validate the candidate run object before writing.
