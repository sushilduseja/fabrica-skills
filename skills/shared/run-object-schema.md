...
Every fab run has one run object. It is created by `fab-intake` and updated only by writer skills according to `skills/manifest.json`.
...
## Required fields created by `fab-intake`
...
| `current_step` | string | `"fab-intake"` on creation; must be a manifest skill id or `null` |
| `current_app_stage` | string or null | Null on creation; if set, must match an `app_stages[].name` value |
| `next_action` | string or null | `"/fab-blueprint"` on creation; skill id must exist in the manifest |
...
| `blueprint_path` | string or null | Null on creation; normally `"docs/blueprint.md"` after `/fab-blueprint` |
...
- `app_stages` — populated by `fab-blueprint`; updated by `fab-frame`, `fab-forge`, `fab-check`, and `fab-trace`.
- `costs` — initialized by `fab-intake`; values remain `unknown` unless measured or estimated evidence exists.
- `verifications` — appended by `fab-forge`, `fab-weave`, `fab-launch`, and `fab-trace`.
...
Canonical source: `skills/manifest.json` (`writes_fields` per skill). Each field is owned by at least one skill. Read-only skills such as `fab-pulse`, `fab-passport`, and `fab-retro` must not write run-object fields.
...
- `/fab-forge <stage>` and `/fab-check <stage>` must reference existing stages;
- `/fab-trace <target>` must reference an existing stage or the special target `integration`.
