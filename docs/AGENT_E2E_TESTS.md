...
## A. fab-trace recovery loop (plan item 29)

1. Drive a run to a failing stage: `/fab-intake` → `/fab-blueprint` →
   `/fab-frame` → `/fab-forge parse-input` where `parse-input` has a failing test.
2. Trigger `/fab-trace parse-input`.
3. Expected, in order:
...
4. Confirm `/fab-passport` modified no run-object field (`fabrica.run.json`
...
1. During a `/fab-forge` implementation, interrupt the agent mid-write.
2. Expected: `fabrica.run.json` is either the pre-run state or fully written —
   ...
   stack trace).
3. If interrupted during a `/fab-passport` write: `docs/handoff.md` is either the
...
## D. fab-pulse rendering across precision states (plan item 33)

For each state, first run `node scripts/validate-run.mjs fabrica.run.json` to
confirm the run object is valid, then run `/fab-pulse`.
...
no file was modified by `/fab-pulse` (it is read-only).
